...
async function findLatestFile(dir, prefix, suffix) {
    const files = (await readdir(dir)).filter(f => f.startsWith(prefix) && f.endsWith(suffix));
    if (files.length === 0)
        return null;
...
let CDP_API_KEY_ID;
let CDP_API_KEY_SECRET;
let CDP_WALLET_SECRET;
try {
...
if (!CDP_API_KEY_ID)
    CDP_API_KEY_ID = await rl.question("Enter CDP_API_KEY_ID: ");
if (!CDP_API_KEY_SECRET)
    CDP_API_KEY_SECRET = await rl.question("Enter CDP_API_KEY_SECRET: ");
if (!CDP_WALLET_SECRET)
    CDP_WALLET_SECRET = await rl.question("Enter CDP_WALLET_SECRET: ");
rl.close();
// 🧪 Validate
